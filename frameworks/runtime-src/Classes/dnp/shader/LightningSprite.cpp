...
void LightningSprite::initProgram(){
    //init shader program
    {
...
bool LightningSprite::setShader(){
...
void LightningSprite::updateNGeneration(){
...
vector<ClineSeg*> LightningSprite::genSegesFromBranchSeg(ClineSeg*branchSeg,int nGeneration)
...
void LightningSprite::genLighting(){
    genSeges();
    genMesh();
}
void LightningSprite::genSeges(){
...
vector<ClineSeg*> LightningSprite::splitSeg(ClineSeg*seg,int generationID)
{
...
void LightningSprite::releaseNeedReGenBranch(vector<ClineSeg*>&branchList,vector<ClineSeg*>&segList){
    int nbranch=(int)branchList.size();
    for(int i=0;i<nbranch;i++){
        ClineSeg*branchSeg=branchList[i];
...
void LightningSprite::releaseNeedReGenBranch_inn(ClineSeg*seg,vector<ClineSeg*>&segList){
    if(seg==NULL)return;
...
void LightningSprite::setStartAndEnd(CCPoint start,CCPoint end){
...
void LightningSprite::debugDraw_segWireFrame(float lineWidth){
...
void LightningSprite::debugDraw_segLineAndDot(float lineWidth,float pointSize){
     //draw segList
     glLineWidth(lineWidth);
...
void LightningSprite::draw(Renderer* renderer, const Mat4 &transform, uint32_t flags){
    
        _customCommand.init(_globalZOrder);
        _customCommand.func = CC_CALLBACK_0(LightningSprite::onDraw,this,transform, flags);
        renderer->addCommand(&_customCommand);
    
    if(m_isDrawDebug){
        
        _customCommand_debug.init(_globalZOrder);
        _customCommand_debug.func = CC_CALLBACK_0(LightningSprite::onDrawDebug,this,transform, flags);
        renderer->addCommand(&_customCommand_debug);
    }
    
}
void LightningSprite::onDraw(const Mat4 &transform, uint32_t flags){
...
void LightningSprite::onDrawDebug(const Mat4 &transform, uint32_t flags){
...
LightningSprite* LightningSprite::create(const char* $pszFileName)
{
    LightningSprite *pobSprite = new LightningSprite();
...
LightningSprite* LightningSprite::create(const char* $pszFileName, const Rect& $rect)
{
    LightningSprite *pobSprite = new LightningSprite();
...
LightningSprite* LightningSprite::createWithSpriteFrame(SpriteFrame* $pSpriteFrame)
{
    LightningSprite *pobSprite = new LightningSprite();
...
LightningSprite* LightningSprite::createWithSpriteFrameName(const char* $pszSpriteFrameName)
{
