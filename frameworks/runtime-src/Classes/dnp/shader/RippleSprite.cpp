...
bool RippleSprite::setShader(){
//    m_gridSideLen=gridSideLen;
    //----init this sprite
//    this->CCSprite::initWithFile(texFileName.c_str());
    //----calculate nRow and nCol
    CCSize contentSize=this->getContentSize();
    m_nRow=floorf(contentSize.height/m_gridSideLen)+1;//add one more row to ensure buffer bigger than background pic
    m_nCol=floorf(contentSize.width/m_gridSideLen)+1;//add one more col to ensure buffer bigger than background pic
...
void RippleSprite::doTouch(const CCPoint&touchPoint,float depth,float r){
...
void RippleSprite::draw(Renderer* renderer, const Mat4 &transform, uint32_t flags){
    ...
    if(m_isDrawDebug){
        ...
        _customCommand_debug.func = CC_CALLBACK_0(RippleSprite::onDrawDebug,this,transform, flags);
...
void RippleSprite::onDrawDebug(const Mat4 &transform, uint32_t flags){
...
RippleSprite* RippleSprite::create(const char* $pszFileName)
{
    RippleSprite *pobSprite = new RippleSprite();
...
RippleSprite* RippleSprite::create(const char* $pszFileName, const Rect& $rect)
{
    RippleSprite *pobSprite = new RippleSprite();
...
RippleSprite* RippleSprite::createWithSpriteFrame(SpriteFrame* $pSpriteFrame)
{
    RippleSprite *pobSprite = new RippleSprite();
...
RippleSprite* RippleSprite::createWithSpriteFrameName(const char* $pszSpriteFrameName)
{
