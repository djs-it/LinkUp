...
--
-- @type Network
local Network = class("HttpNetwork",require("util.eventable").new())
...
function Network:ctor()
    ...
    self.bapool = require("util.objpool").new():createMore("fmw.network.ByteArray",20)
    self.reconnect = true;
end
...
--@param self
--@param string#string url http url
...
function Network:connect(url)
    ...
    local http = require("fmw.network.impl.Http")
    self.socket = http.new(url, self.reconnect)

    self.socket:addEventListener(http.EVENT_CONNECTED, onConnected)
    self.socket:addEventListener(http.EVENT_CLOSE, onClose)
    self.socket:addEventListener(http.EVENT_CLOSED, onStatus)
    self.socket:addEventListener(http.EVENT_CONNECT_FAILURE, onConnectFail)
    self.socket:addEventListener(http.EVENT_DATA, onData)
    self.socket:connect()
...
---------------------------
--判断网络是否联通
--@function [parent=#Network] getConnected
--@param self
